...
function WellnessTrendChart({ trend, suppressed }: { trend: TrendPoint[]; suppressed: boolean }) {
  ...
  const chartData = trend.map((pt) => ({
    name: monthLabel(pt.month),
    score: pt.avgScore,
  }));

  const hasData = trend.some((pt) => pt.avgScore != null);
  ...
  function DeltaBadge({ d, unit }: { d: number | null; unit: string }) {
    ...
    return (
      <span style={{
        ...
        {up ? "+" : "-"}{Math.abs(d).toFixed(1)}{unit} {t("employer.roi.benchmark.vsPlatform")}
      </span>
    );
  }
  ...
  return (
    <div style={{ background: "white", border: "1.5px solid rgba(212,34,126,0.1)", borderRadius: 12, padding: "24px 20px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
        ...
        <h3 style={{ fontFamily: "var(--app-font-sans)", fontSize: 14, fontWeight: 700, color: navy, margin: 0 }}>
          {t("employer.roi.benchmark.heading")}
        </h3>
      </div>
      <p style={{ fontFamily: "var(--app-font-sans)", fontSize: 12, color: "var(--text-muted)", margin: "0 0 20px" }}>
        {t("employer.roi.benchmark.sub")}
        {platform.sampleSize && ` ${t("employer.roi.benchmark.sampleNote", { n: platform.sampleSize })}`}
      </p>
      ...
      {privacySuppressed ? (
        <div style={{ textAlign: "center", padding: "24px 0" }}>
          ...
          <p style={{ fontFamily: "var(--app-font-sans)", fontSize: 13, color: "var(--text-secondary)", marginTop: 8 }}>
            {t("employer.roi.benchmark.suppressedMsg")}
          </p>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          {[
            {
              label: t("employer.roi.benchmark.avgWellness"),
            ...
            {
              label: t("employer.roi.benchmark.utilRate"),
            ...
            <div key={row.label}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <div style={{ background: "rgba(212,34,126,0.04)", border: "1px solid rgba(212,34,126,0.1)", borderRadius: 8, padding: "12px 16px" }}>
                  <div style={{ fontFamily: "var(--app-font-sans)", fontSize: 11, fontWeight: 600, color: "var(--text-muted)", textTransform: "uppercase" as const, letterSpacing: "0.06em", marginBottom: 4 }}>{t("employer.roi.benchmark.yourCompany")}</div>
                ...
                <div style={{ background: "rgba(107,132,153,0.05)", border: "1px solid rgba(107,132,153,0.1)", borderRadius: 8, padding: "12px 16px" }}>
                  <div style={{ fontFamily: "var(--app-font-sans)", fontSize: 11, fontWeight: 600, color: "var(--text-muted)", textTransform: "uppercase" as const, letterSpacing: "0.06em", marginBottom: 4 }}>{t("employer.roi.benchmark.platformAvg")}</div>
      ...
      <p style={{ margin: "16px 0 0", fontFamily: "var(--app-font-sans)", fontSize: 11, color: "var(--text-muted)", lineHeight: 1.5 }}>
        {t("employer.roi.benchmark.privacyNote")}
      </p>
    </div>
  );
}
...
function RoiCalculator({ defaultStipendCents }: { defaultStipendCents: number }) {
  ...
  const fields = [
    { key: "headcount" as const, label: t("employer.roi.calculator.enrolledEmployees"), prefix: "", suffix: "", min: 1, hint: "" },
    { key: "avgSalary" as const, label: t("employer.roi.calculator.avgSalary"), prefix: "$", suffix: "", min: 0, hint: "" },
    { key: "sickDaysReduced" as const, label: t("employer.roi.calculator.sickDaysReduced"), prefix: "", suffix: " days", min: 0, hint: t("employer.roi.calculator.sickDaysHint") },
    { key: "productivityPct" as const, label: t("employer.roi.calculator.productivityPct"), prefix: "", suffix: "%", min: 0, hint: t("employer.roi.calculator.productivityHint") },
    { key: "annualStipendCents" as const, label: t("employer.roi.calculator.annualStipend"), prefix: "$", suffix: ` ${t("employer.roi.calculator.annualStipendSuffix")}`, min: 0, hint: "" },
  ] as const;
  ...
  return (
    <div style={{ background: "white", border: "1.5px solid rgba(212,34,126,0.1)", borderRadius: 12, padding: "24px 20px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
        ...
        <h3 style={{ fontFamily: "var(--app-font-sans)", fontSize: 14, fontWeight: 700, color: navy, margin: 0 }}>
          {t("employer.roi.calculator.heading")}
        </h3>
      </div>
      <p style={{ fontFamily: "var(--app-font-sans)", fontSize: 12, color: "var(--text-muted)", margin: "0 0 20px" }}>
        {t("employer.roi.calculator.sub")}
      </p>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12, marginBottom: 20 }}>
        {[
          { label: t("employer.roi.calculator.sickDaySavings"), value: fmtDollar(sickDaySavings), sub: t("employer.roi.calculator.sickDaySub", { days: inputs.sickDaysReduced, employees: inputs.headcount }), color: sage },
          { label: t("employer.roi.calculator.productivityGain"), value: fmtDollar(productivityGain), sub: t("employer.roi.calculator.productivitySub", { pct: inputs.productivityPct.toFixed(1) }), color: "#4a90e2" },
          { label: t("employer.roi.calculator.totalValue"), value: fmtDollar(totalValue), sub: t("employer.roi.calculator.totalValueSub"), color: navy },
          {
            label: t("employer.roi.calculator.netROI"),
            value: `${netROIPct >= 0 ? "+" : ""}${netROIPct}%`,
            sub: t("employer.roi.calculator.netROISub"),
      ...
      <div style={{
        ...
        <p style={{ fontFamily: "var(--app-font-sans)", fontSize: 11, color: "var(--text-muted)", margin: 0, lineHeight: 1.6 }}>
          {t("employer.roi.calculator.disclaimer")}
        </p>
      </div>
    </div>
  );
