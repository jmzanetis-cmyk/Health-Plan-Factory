...
function readSurveyData(): SurveyData | null {
  try {
...
export function PlanSpeculator() {
  ...
  const sd = useRef<SurveyData | null>(readSurveyData()).current;
  const isSurveyPrefilled = Boolean(sd?.budgetMidpoint || (sd?.goals && sd.goals.length > 0));
  const autoGeneratedRef = useRef(false);

  const [budget, setBudget] = useState<number>(() => {
    if (sd?.budgetMidpoint) return sd.budgetMidpoint;
    return 250;
  });
  const [conditions, setConditions] = useState<string[]>([]);
  const [conditionWeights, setConditionWeights] = useState<Record<string, ConditionWeight>>({});
  const [goals, setGoals] = useState<string[]>(() => {
    if (sd?.goals && sd.goals.length > 0) return sd.goals;
    return [];
  });
  ...
  useEffect(() => {
    if (isSurveyPrefilled && budget > 0 && goals.length > 0 && !autoGeneratedRef.current) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {isSurveyPrefilled
            ? "We used your survey answers to build this — adjust anything and regenerate."
